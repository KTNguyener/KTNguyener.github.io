function Snake() {//constructs the snake
  this.x=0;//this refers to the Snake, or whatever is above it.
  this.y=0;//sets the default coordinates
  ...
  this.death = function() {
    for (var i = 0; i<this.tail.length; i++) {
      ...
      if ((d<1) || ((this.x<0) || (this.x>w-scl) || (this.y<0) || (this.y>h-scl))) { //Or X or Y exceed the canvas
        this.total=0; //Reset the score, clear the total
